...
fn find_regex(winners: &mut Set, losers: &Set) -> String {
    ...
    while !winners.is_empty() {
        let best = covers.iter().max_by_key(|(reg, matching)| {
            4 * matching.intersection(winners).count() as i64 - reg.cost() as i64
        });
        if let Some((part, matched)) = best {
            solutions.push(part.clone());
            winners.retain(|x| !matched.contains(x));
            covers.retain(|_, matched| matched.intersection(winners).next().is_some());
        } else {
...
fn bench(){
    ...
    eprintln!("{}", find_regex(&mut winners, &losers));
    ...
    eprintln!("{}", find_regex(&mut boys, &girls));
    ...
    eprintln!("{}", find_regex(&mut pharma, &cities));
    ...
    eprintln!("{}", find_regex(&mut foo, &bar));
    ...
    eprintln!("{}", find_regex(&mut nouns, &adverbs));
    ...
    eprintln!("{}", find_regex(&mut randoms, &builtins));
    ...
    eprintln!("{}", find_regex(&mut starwars, &startrek));
    ...
    eprintln!("{}", find_regex(&mut dogs, &cats));
    ...
    eprintln!("{}", find_regex(&mut movies, &tv));
    ...
    eprintln!("{}", find_regex(&mut stars, &scientists));
    }
