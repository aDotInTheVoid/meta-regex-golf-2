...
def output(vals, name):
    print(f"let mut {name}: Set = [{','.join(map(lambda x: DOUBLE+x+DOUBLE, vals))}].iter().copied().collect();")#": Set = &[{",".join(map(repr, vals))}].iter().collect()")

def output_lest(wname, lname):
    # print(println!("{}", find_regex(&mut winners, &losers));)

    print(f"println!(\"{'{}'}\", find_regex(&mut {wname}, &{lname}));")

def overall(wname, lname):
    output(globals()[wname], wname)
    output(globals()[lname], lname)
    output_lest(wname, lname)
...
overall("nouns", "adverbs")

randoms = Set(vars(random))
builtins = Set(vars(__builtins__)) - randoms

overall("randoms", "builtins")
...
scientists = phrases('''Alain Aspect / Martin Karplus / David Baltimore / Donald Knuth / Allen Bard / 
    ...
    Gordon Moore / Sydney Brenner / Kary Mullis / Pierre Chambon / C Nusslein-Volhard / Simon Conway Morris / 
    Seiji Ogawa / Mildred Dresselhaus / Jeremiah Ostriker / Gerald M Edelman / Roger Penrose / Ronald Evans / 
    Stanley Prusiner / Anthony Fauci / Henry F Schaefer III / Anthony Fire / Thomas Sudhof / Jean Frechet / 
